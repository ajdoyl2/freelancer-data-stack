...
# from dagster_datahub import datahub_resource  # Import disabled for compatibility
...
def airbyte_sync_job(
    context: AssetExecutionContext, config: AirbyteConfig
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        context.add_output_metadata(
            {
                "records_extracted": MetadataValue.int(
                    sync_metadata["records_extracted"]
                ),
...
def dlt_ingestion_pipeline(
    context: AssetExecutionContext, config: DltConfig
) -> dict[str, Any]:
    """
        ...
        context.add_output_metadata(
            {
                ...
                "records_loaded": MetadataValue.int(
                    pipeline_metadata.get("records_loaded", 0)
                ),
...
def dbt_test(context: AssetExecutionContext, config: DbtConfig) -> dict[str, Any]:
    ...
    try:
        ...
        test_metadata = {
            "status": (
                "completed" if result.returncode == 0 else "completed_with_failures"
            ),
            "target": config.target,
            "stdout": result.stdout,
...
def great_expectations_validation(
    context: AssetExecutionContext, config: GreatExpectationsConfig
) -> dict[str, Any]:
    """
        ...
        context.add_output_metadata(
            {
                ...
                "validation_success": MetadataValue.bool(
                    validation_metadata.get("validation_success", False)
                ),
                "failed_expectations": MetadataValue.int(
                    validation_metadata.get("failed_expectations", 0)
                ),
            }
        )
...
def datahub_metadata_ingestion(context: AssetExecutionContext) -> dict[str, Any]:
    ...
    try:
        ...
        for config_path in ingestion_configs:
            ...
            datahub_command = ["datahub", "ingest", "-c", config_path]
            ...
            if result.returncode != 0:
                logger.error(
                    f"DataHub ingestion failed for {config_path}: {result.stderr}"
                )
        ...
        successful_configs = len(
            [r for r in ingestion_results if r["status"] == "completed"]
        )
        ...
        logger.info(
            f"DataHub metadata ingestion completed: {successful_configs}/{total_configs} configs successful"
        )
