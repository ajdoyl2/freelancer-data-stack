...
class MemoryEnabledAgent:
    ...
    def start_conversation(
        self, context_summary: str = None, tags: list[str] = None
    ) -> str:
    ...
    def end_conversation(self, summary: str = None):
        ...
        if self.current_conversation_id:
            self.memory_manager.end_conversation(
                self.current_conversation_id, context_summary=summary
            )
            self.current_conversation_id = None
    ...
    def remember(
        ...
        return self.memory_manager.add_memory_fragment(
            fragment=fragment, source_conversation_id=self.current_conversation_id
        )
    ...
    def add_reasoning(
        self, reasoning_steps: list[ReasoningStep], message_id: str = None
    ):
    ...
    def get_conversation_context(self, limit: int = 3) -> str:
        ...
        recent_convos = self.memory_manager.get_conversation_history(
            agent_name=self.agent_name, user_id=self.user_id, limit=limit
        )
        ...
        if relevant_memories:
            ...
            for memory in relevant_memories:
                context_parts.append(
                    f"- [{memory['fragment_type']}] {memory['content']}"
                )
    ...
    def extract_insights(self, conversation_content: str) -> list[MemoryFragment]:
        ...
        if (
            "always" in conversation_content.lower()
            or "never" in conversation_content.lower()
        ):
            insights.append(
                MemoryFragment(
                    fragment_type="rule",
...
class WarpAgentMemory(MemoryEnabledAgent):
    ...
    def __init__(self):
        super().__init__(
            agent_name="warp_agent", user_id=os.getenv("USER", "default_user")
        )
    ...
    def remember_user_workflow(
        self, workflow_name: str, steps: list[str], context: str = ""
    ):
    ...
    def get_command_suggestions(self, current_context: str) -> list[str]:
        ...
        for memory in command_memories:
            ...
            if "Successful command:" in content:
                cmd_line = (
                    content.split("Successful command:")[1].split("\n")[0].strip()
                )
    ...
    def get_workflow_suggestions(self, task_description: str) -> list[dict[str, Any]]:
        ...
        workflow_memories = self.recall(
            query=task_description, fragment_type="pattern", tags=["workflow"], limit=3
        )
