...
class AgentMemoryManager:
    ...
    def start_conversation(self, session_id: str, agent_name: str, 
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, [
                conversation_id, session_id, agent_name, user_id,
                context_summary, tags or [], json.dumps(metadata or {})
            ])
    ...
    def add_message(self, conversation_id: str, role: str, content: str,
                   tokens_used: int = None, reasoning_trace: str = None,
                   tool_calls: List[Dict] = None, metadata: Dict[str, Any] = None) -> str:
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            ...
            """, [
                ...
                reasoning_trace, json.dumps(tool_calls or []), json.dumps(metadata or {})
            ])
        
        return message_id
    
    def add_memory_fragment(self, fragment: MemoryFragment, 
                           source_conversation_id: str = None,
                           source_message_id: str = None) -> str:
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, [
                fragment_id, fragment.fragment_type, fragment.content,
                source_conversation_id, source_message_id,
                fragment.confidence_score, fragment.importance_score,
                fragment.tags, json.dumps(fragment.metadata)
            ])
        
        return fragment_id
    
    def add_reasoning_trace(self, conversation_id: str, steps: List[ReasoningStep],
                           message_id: str = None):
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            for step in steps:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, [
                    conversation_id, message_id, step.step_number, step.reasoning_type,
                    step.thought, step.action_taken, step.outcome, step.confidence,
                    json.dumps(step.metadata)
                ])
    
    def set_agent_state(self, agent_name: str, state_key: str, state_value: Any,
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            conn.execute("""
                ...
                ON CONFLICT (agent_name, state_key) DO UPDATE SET
                    ...
                    updated_at = ?
            """, [agent_name, state_key, json.dumps(state_value), expiry_time, datetime.now(), datetime.now()])
    ...
    def search_memory_fragments(self, query: str = None, fragment_type: str = None,
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            ...
            return [
                {
                    "id": row[0], "fragment_type": row[1], "content": row[2],
                    "confidence_score": row[3], "importance_score": row[4],
                    "tags": row[5], "created_at": row[6], "last_accessed": row[7],
                    "access_count": row[8], "metadata": json.loads(row[9] or "{}")
                }
                for row in results
            ]
    
    def get_conversation_history(self, agent_name: str = None, user_id: str = None,
            ...
            return [
                {
                    ...
                    "user_id": row[3], "start_time": row[4], "end_time": row[5],
                    "context_summary": row[6], "tags": row[7],
    ...
    def get_reasoning_traces(self, conversation_id: str) -> List[Dict[str, Any]]:
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            ...
            return [
                {
                    "step_number": row[0], "reasoning_type": row[1], "thought": row[2],
                    "action_taken": row[3], "outcome": row[4], "confidence": row[5],
                    "timestamp": row[6], "metadata": json.loads(row[7] or "{}")
                }
                for row in results
            ]
    
    def set_user_context(self, user_id: str, context_type: str, context_key: str,
                        context_value: Any, priority: int = 5):
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            conn.execute("""
                ...
                ON CONFLICT (user_id, context_type, context_key) DO UPDATE SET
                    ...
                    updated_at = ?
            """, [user_id, context_type, context_key, json.dumps(context_value), priority, datetime.now(), datetime.now()])
    
    def get_user_context(self, user_id: str, context_type: str = None) -> Dict[str, Any]:
