...
from memory_manager import AgentMemoryManager, MemoryFragment
from agent_integration import WarpAgentMemory, get_warp_memory
...
def cmd_export(args):
    ...
    filename = args.output if hasattr(args, 'output') else f"agent_memory_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
Examples:
  ...
  memory export                   # Export all memory data
    ...
    search_parser.add_argument('--min-importance', type=float, default=0.0, help='Minimum importance score')
    
    # Conversations command
    conv_parser = subparsers.add_parser('conversations', help='Show recent conversations')
    ...
    remember_parser.add_argument('type', choices=['fact', 'pattern', 'preference', 'rule', 'context'], help='Fragment type')
    remember_parser.add_argument('content', help='Content to remember')
    remember_parser.add_argument('--importance', type=float, default=0.7, help='Importance score (0-1)')
