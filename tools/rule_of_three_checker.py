...
class RuleOfThreeChecker:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if end_line - start_line + 1 >= self.min_lines:
                    ...
                    ast_hash = hashlib.sha256(normalized.encode()).hexdigest()
    ...
    def calculate_similarity(self, frag1: CodeFragment, frag2: CodeFragment) -> float:
        ...
        matcher = difflib.SequenceMatcher(
            None, frag1.normalized_content, frag2.normalized_content
        )
    ...
    def find_duplication_groups(self) -> list[DuplicationGroup]:
        ...
        for _ast_hash, fragments in hash_groups.items():
        ...
        for i, frag1 in enumerate(remaining_fragments):
            ...
            for _j, frag2 in enumerate(remaining_fragments[i + 1 :], i + 1):
                ...
                if (
                    ...
                    and frag1.fragment_type == frag2.fragment_type
                ):
            ...
            if len(similar_group) >= 3:  # Rule of Three
                group = DuplicationGroup(
                    ...
                    similarity_score=min(
                        self.calculate_similarity(similar_group[0], f)
                        for f in similar_group[1:]
                    ),
    ...
    def _calculate_impact_score(self, fragments: list[CodeFragment]) -> float:
        ...
        avg_size = (
            sum(f.end_line - f.start_line + 1 for f in fragments) / num_duplicates
        )
    ...
    def _generate_text_report(self) -> str:
        ...
        if not self.duplication_groups:
            ...
            report.append(
                "All code appears to follow good DRY (Don't Repeat Yourself) principles."
            )
            return "\n".join(report)

        report.append(
            f"⚠️  Found {len(self.duplication_groups)} duplication groups that violate the Rule of Three"
        )
        report.append("")

        for i, group in enumerate(self.duplication_groups, 1):
            report.append(
                f"## Duplication Group {i} (Impact: {group.impact_score:.1f}/10)"
            )
            ...
            for j, fragment in enumerate(group.fragments, 1):
                location = (
                    f"{fragment.file_path}:{fragment.start_line}-{fragment.end_line}"
                )
                if fragment.name:
        ...
        report.append(
            f"- Average impact score: {sum(g.impact_score for g in self.duplication_groups) / len(self.duplication_groups):.1f}"
        )
    ...
    def _generate_json_report(self) -> str:
        ...
        report_data = {
            "summary": {
                ...
                "total_fragments": sum(
                    len(g.fragments) for g in self.duplication_groups
                ),
                "high_impact_groups": len(
                    [g for g in self.duplication_groups if g.impact_score >= 5.0]
                ),
                "average_impact": (
                    sum(g.impact_score for g in self.duplication_groups)
                    / len(self.duplication_groups)
                    if self.duplication_groups
                    else 0
                ),
            },
            "duplication_groups": [
...
Examples:
  ...
  python rule_of_three_checker.py . --exclude test_,migrations  # Exclude patterns
    ...
    parser.add_argument("directory", help="Directory to analyze for duplicate code")
    ...
    if args.fail_on_duplication and checker.duplication_groups:
        high_impact_groups = [
            g for g in checker.duplication_groups if g.impact_score >= args.max_impact
        ]
        if high_impact_groups:
            print(
                f"\n❌ Found {len(high_impact_groups)} high-impact duplication groups (threshold: {args.max_impact})",
                file=sys.stderr,
            )
            sys.exit(1)
        else:
            print(
                f"\n⚠️  Found duplications but all below impact threshold ({args.max_impact})",
                file=sys.stderr,
            )
