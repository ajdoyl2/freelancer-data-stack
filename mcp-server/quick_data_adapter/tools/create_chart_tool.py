    ...
    try:
        ...
        if chart_type == "histogram":
            ...
            if not y_column:
                ...
                if groupby_column:
                    agg_data = (
                        df.groupby([x_column, groupby_column])[y_column]
                        .mean()
                        .unstack(fill_value=0)
                    )
            ...
            fig = px.scatter(
                df, x=x_column, y=y_column, color=groupby_column, title=title
            )
            ...
            if groupby_column:
                fig = px.line(
                    df_sorted, x=x_column, y=y_column, color=groupby_column, title=title
                )
            else:
            ...
            return {
                "error": f"Unsupported chart type: {chart_type}. Supported: histogram, bar, scatter, line, box"
            }
        ...
        if save_path or fig:
            if save_path is None:
                ...
                save_path = (
                    outputs_dir / f"chart_{dataset_name}_{chart_type}_{x_column}.html"
                )
