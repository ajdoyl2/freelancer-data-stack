        ...
        for config in dataset_configs[1:]:
            ...
            if join_column:
                ...
                if join_column not in merged_df.columns:
                    return {
                        "error": f"Join column '{join_column}' not found in merged dataset"
                    }
                if join_column not in df_to_merge.columns:
                    return {
                        "error": f"Join column '{join_column}' not found in dataset '{dataset_name}'"
                    }
                ...
                before_shape = merged_df.shape
                merged_df = pd.concat(
                    [merged_df, df_to_merge], ignore_index=True, sort=False
                )
                after_shape = merged_df.shape
