...
async def export_insights(
    ...
    try:
        ...
        insights = {
            ...
            "data_quality": {
                "missing_data_columns": len(
                    [c for c in df.columns if df[c].isnull().any()]
                ),
        ...
        if format.lower() == "json":
            ...
            summary_data.append(
                ["Memory Usage (MB)", insights["dataset_info"]["memory_usage_mb"]]
            )
            summary_data.append(
                ["Numerical Columns", insights["schema_summary"]["numerical_columns"]]
            )
            summary_data.append(
                [
                    "Categorical Columns",
                    insights["schema_summary"]["categorical_columns"],
                ]
            )
            summary_data.append(
            ...
            return {
                "error": f"Unsupported export format: {format}. Use 'json', 'csv', or 'html'"
            }
