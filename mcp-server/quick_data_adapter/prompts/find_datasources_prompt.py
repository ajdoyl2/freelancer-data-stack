...
"""
        ...
        if csv_files or json_files:
            prompt += "**📊 Data files found in current directory:**\n\n"

            all_current_files = sorted(
                csv_files + json_files, key=lambda x: x.name.lower()
            )
            for file_path in all_current_files:
                ...
                suggested_name = (
                    file_path.stem.lower().replace(" ", "_").replace("-", "_")
                )
        ...
        # Subdirectory files
        if subdir_files:
            prompt += "**📂 Data files found in subdirectories:**\n\n"
            ...
            for subdir_name, files in subdir_files:
                ...
                for file_path in sorted_files:
                    ...
                    prompt += f"  • **{file_path.name}** ({file_type}, {size_str})\n"
                    prompt += (
                        f"    → `load_dataset('{file_path}', '{suggested_name}')`\n"
                    )
...
"""
            ...
            total_files = (
                len(csv_files)
                + len(json_files)
                + sum(len(files) for _, files in subdir_files)
            )
