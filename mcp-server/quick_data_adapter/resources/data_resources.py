...
from ..models.schemas import DatasetManager, UserProfile, dataset_schemas
            ...
            datasets.append(
                {
                    ...
                    "column_types": {
                        role: len(
                            [
                                c
                                for c, col_info in info["schema"]["columns"].items()
...
async def get_available_analyses(dataset_name: str | None = None) -> dict:
            ...
            available_analyses.append(
                {
                    ...
                    "applicable_columns": {
                        "date_columns": temporal_cols,
                        "value_columns": numerical_cols,
                    },
                }
            )
...
async def get_column_types(dataset_name: str | None = None) -> dict:
...
async def get_analysis_suggestions(dataset_name: str | None = None) -> dict:
            ...
            usage.append(
                {
                    ...
                    "columns": info["shape"][1],
                    "memory_per_row_kb": (
                        round(memory_mb * 1024 / info["shape"][0], 2)
                        if info["shape"][0] > 0
                        else 0
                    ),
                }
...
async def get_system_status() -> dict:
    ...
    datasets = DatasetManager.list_datasets()
    total_memory = sum(
        DatasetManager.get_dataset_info(name)["memory_usage_mb"] for name in datasets
    )
