...
class DatasetSchema(BaseModel):
    ...
    def from_dataframe(cls, df: pd.DataFrame, name: str) -> "DatasetSchema":
        ...
        suggestions = []
        numerical_cols = [
            col for col, info in columns.items() if info.suggested_role == "numerical"
        ]
        categorical_cols = [
            col for col, info in columns.items() if info.suggested_role == "categorical"
        ]
        temporal_cols = [
            col for col, info in columns.items() if info.suggested_role == "temporal"
        ]
...
class DatasetManager:
    ...
    def get_dataset(dataset_name: str) -> pd.DataFrame:
        ...
        if dataset_name not in loaded_datasets:
            raise ValueError(
                f"Dataset '{dataset_name}' not loaded. Use load_dataset() first."
            )
    ...
    def clear_dataset(dataset_name: str) -> dict:
        ...
        return {
            "status": "success",
            "message": f"Dataset '{dataset_name}' cleared from memory",
        }
