...
from ..models.schemas import DataQualityReport, DatasetManager, dataset_schemas
...
async def validate_data_quality(dataset_name: str) -> dict:
    ...
    try:
        ...
        if duplicate_rows > 0:
            ...
            recommendations.append(
                "Remove duplicate rows or investigate if duplicates are intentional"
            )

        # Potential ID columns that aren't unique
        for col_name, col_info in schema.columns.items():
            if (
                col_info.suggested_role == "identifier"
                and col_info.unique_values < len(df)
            ):
                issues.append(
                    f"Column '{col_name}' appears to be an ID but has duplicates"
                )
                recommendations.append(
                    f"Investigate duplicate values in '{col_name}' column"
                )

        # Mixed data types in object columns
        object_cols = df.select_dtypes(include=["object"]).columns
        for col in object_cols:
            sample_types = {type(x).__name__ for x in df[col].dropna().head(100)}
        ...
        score -= (
            len([col for col, pct in missing_data.items() if pct > 10]) * 10
        )  # High missing penalty
        score = max(0, score)
