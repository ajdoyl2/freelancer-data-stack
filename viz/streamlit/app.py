...
def show_duckdb_analytics():
    ...
    if not df.empty:
        ...
        st.warning(
            "No data available in DuckDB. Please check your database connection."
        )
...
def show_snowflake_analytics():
    ...
    st.info(
        "🚧 Snowflake connection setup required. This page will display Snowflake analytics once configured."
    )
    ...
    fig = px.bar(
        ...
        x="Schema",
        y=["Tables", "Views"],
        title="Snowflake Schema Overview",
        barmode="group",
    )
    st.plotly_chart(fig, use_container_width=True)
...
def show_data_quality():
    ...
    quality_scores = 95 + 5 * pd.Series(range(len(dates))).apply(
        lambda x: 0.01 * x % 10
    )

    quality_df = pd.DataFrame({"Date": dates, "Quality Score": quality_scores})

    fig = px.line(
        quality_df, x="Date", y="Quality Score", title="Data Quality Score Over Time"
    )
...
def show_system_monitoring():
    ...
    with col1:
        ...
        cpu_data = pd.DataFrame(
            {"Service": services["Service"], "CPU %": [12, 8, 15, 5, 20, 18]}
        )

        fig = px.pie(
            cpu_data, values="CPU %", names="Service", title="CPU Usage by Service"
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        memory_data = pd.DataFrame(
            {"Service": services["Service"], "Memory MB": memory_values}
        )

        fig = px.bar(
            memory_data, x="Service", y="Memory MB", title="Memory Usage by Service"
        )
        st.plotly_chart(fig, use_container_width=True)
